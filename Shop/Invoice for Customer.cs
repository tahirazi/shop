...
namespace Shop
{
    public partial class Invoice_for_Customer : Form
    {
        ...
        private void Invoice_for_Customer_Load(object sender, EventArgs e)
        {
        ...
        public void debitCredit()
        {
            try
            {
                ...
                cmd = new SqlCommand();
                cmd.Connection = con;
                cmd.CommandText = "select sum(balance) as ABC from balances where id='" + Convert.ToInt32(txtCustomerID.Text) + "'";
                SqlDataReader rdr = cmd.ExecuteReader();
                DataTable dt = new DataTable("balances");
                dt.Load(rdr);
                if (dt.Rows.Count > 0)
                {
        ...
        private void Save_Click(object sender, EventArgs e)
        {
            ...
            {
                string cd = "insert Into soldc(Invoice_No,Config_ID,Quantity,Price,Total,soldby) VALUES (@InvoiceNo,@ConfigID,@Quantity,@Price,@Totalamount,@soldby)";
        ...
        {
            Global.customerInvoicePrint++;
            if (Global.customerInvoicePrint == 1)
            {
                Customer_Invoice_Print frm = new Customer_Invoice_Print();
                Customer_Invoice_Print.abc = txtInvoiceNo.Text;
        ...
        private void cmbCustomerID_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = "select id,customer_name from customers where id='" + Convert.ToInt32(cmbCustomerID.Text) + "'";
            SqlDataReader rdr = cmd.ExecuteReader();
        ...
        public void updateBalance()
        {
            ...
            {
            con.Open();
            string query = "INSERT INTO balances VALUES ('" + Convert.ToInt32(txtCustomerID.Text) + "','" + txtInvoiceNo.Text + "','" + dtpInvoiceDate.Text + "','" + Convert.ToDecimal(txtTotal.Text) + "','" + Convert.ToDecimal(txtCreditDebit.Text) + "')";
            cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = query;
            cmd.ExecuteNonQuery();
        ...
        private void Invoice_for_Customer_FormClosing(object sender, FormClosingEventArgs e)
        {
            Global.customerInvoice = 0;
        }
